...
class DoLogin(webapp.RequestHandler):
  def post(self):
    ...
    try:
      ...
      self.redirect('/?SAMLRequest='+urllib.quote(self.request.get('SAMLRequest'))+'&RelayState='+urllib.quote(self.request.get('RelayState'))+'&Error=Your%20account%20is%20locked.%20%3Ca%20href%3D%22https%3A//www.google.com/a/'+domain+'/DisplayUnlockCaptcha%22%3EClick%20here%20to%20unlock%20it.%3C/a%3E')
    if becomeattempt:
